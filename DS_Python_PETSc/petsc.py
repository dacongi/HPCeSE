#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#! petsc.py: Power System Dynamic Simulation Application(AB Version)
#! Author: Liwei Wang 
...
# Read Input Data
    ...
    #Start to compute the dynamic simulation
    ...
    for I_Steps in range(1,simu_k+2):
        if I_Steps<steps1:
            S_Steps=I_Steps
            flagF1=0
        elif I_Steps==steps1:
        ...
        if I_Steps>1 and I_Steps<simu_k+2:
            mac_ang_s1.copy(mac_ang_s0)
            #mac_ang_s0.view()
            #eprime_s1.copy(eprime_s0)
            dmac_ang_s1.copy(dmac_ang_s0)
            dmac_spd_s1.copy(dmac_spd_s0)
        ...
        h.copy(vecTemp1)
        vecTemp1.scale(2)
        dmac_spd_s1.pointwiseDivide(dmac_spd_s1,vecTemp1)
        dmac_ang_s1.copy(dmac_ang_s0)
        dmac_spd_s1.copy(dmac_spd_s0)

        if S_Steps==1:

            #k2_mac_ang and k2_mac_spd
            k1_mac_ang.axpy(h_sol1,dmac_ang_s0)
            k1_mac_spd.axpy(h_sol1,dmac_spd_s0)

            #k2_mac_ang
            mac_spd_s0.copy(vecTemp1)
            vecTemp1.axpy(1,k1_mac_ang)
            vecTemp1.shift(-1)
            vecTemp1.scale(basrad)
            vecTemp1.scale(h_sol1)
            vecTemp1.copy(k2_mac_ang)

            #k2_mac_spd
            pmech.copy(vecTemp1)
            vecTemp1.axpy(-1,pelect)
            
            mac_spd_s0.copy(vecTemp2)
            vecTemp2.axpy(1,k1_mac_spd)
            vecTemp2.shift(-1)
            vecTemp2.pointwiseMult(d0,vecTemp2)
            vecTemp1.axpy(-1,vecTemp2)
            vecTemp1.scale(h_sol1)

            h.copy(vecTemp2)
            vecTemp2.scale(2)
            k2_mac_spd.pointwiseDivide(vecTemp1,vecTemp2)
            #k2_mac_ang.view()
            #k2_mac_spd.view()

            # mac_ang_s1
            mac_ang_s0.copy(vecTemp1)
            k1_mac_ang.copy(vecTemp2)
            vecTemp2.axpy(1,k2_mac_ang)
            vecTemp2.scale(h_sol1/2)
            vecTemp1.axpy(1,vecTemp2)
            vecTemp1.copy(mac_ang_s1)
         
            # mac_spd_s1
            mac_spd_s0.copy(vecTemp1)
            k1_mac_spd.copy(vecTemp2)
            vecTemp2.axpy(1,k2_mac_spd)
            vecTemp2.scale(h_sol1/2)
            vecTemp1.axpy(1,vecTemp2)
            vecTemp1.copy(mac_spd_s1)
            ...
            mac_spd_s0.copy(vecTemp2)
            vecTemp2.shift(-1.0)
            vecTemp2.scale(3*basrad)
            vecTemp2.axpy(-1,dmac_ang_s0)
            vecTemp2.scale(h_sol1/2.0)
            vecTemp1.axpy(1,vecTemp2)
            vecTemp1.copy(mac_ang_s1)
            ...
            vecTemp1.pointwiseDivide(vecTemp1,vecTemp2)
            vecTemp1.axpy(-1,dmac_spd_s0)
            vecTemp1.scale(h_sol1/2)
            mac_spd_s0.copy(vecTemp2)
            vecTemp2.axpy(1,vecTemp1)
            vecTemp2.copy(mac_spd_s1)
