...
def reduce_y(flag):
    ...
    #start_ysparse = MPI.Wtime()
    x=y_sparse(flag)
    #end_ysparse = MPI.Wtime()
    #comm.Barrier()
    #if rank == 0:
        #print(end_ysparse-start_ysparse)
    if flag == 0:
        busy=recv_ipt_bus
        liney=recv_ipt_brch
        Y_d=x
...
for i in range(nsw-1):
    t_step[i]=round((s1[i+1]-s1[i])/s7[i])
    t_width[i]=(s1[i+1]-s1[i])/t_step[i]
    sim_k=sim_k+t_step[i]

sim_k=sim_k+1

# Initialize simulation variables
mac_ang,mac_spd,dmac_ang,dmac_spd,edprime,eqprime,pmech,pelect=np.zeros((8,array_sizes_gen[rank],sim_k),dtype=np.float64)
...
edprime[:,0]=eprime[:,0].real
eqprime[:,0]=eprime[:,0].imag
curr=curr*rot    
curdg=curr.real
curqg=curr.imag
curd=curr.real/mva
...
for I_Steps in range(1,sim_k+2):
    ...
    pmech[:,S_Steps] = pmech[:,S_Steps-1]
    eprime[:,S_Steps-1] = np.sin(mac_ang[:,S_Steps-1])*edprime[:,S_Steps-1]+np.cos(mac_ang[:,S_Steps-1])*eqprime[:,S_Steps-1]-jay*(np.cos(mac_ang[:,S_Steps-1])*edprime[:,S_Steps-1]-np.sin(mac_ang[:,S_Steps-1])*eqprime[:,S_Steps-1])
    
    #comm.Barrier()
    #a=MPI.Wtime()
    if rank == 0:
        recvdata = eprime[:,S_Steps-1]
        for i in range(1,size):
            recvdata = np.concatenate((recvdata, comm.recv(source=i)),axis=0)
        recvdata = comm.bcast(recvdata,root=0) 
    else:
        comm.send(eprime[:,S_Steps-1],dest=0)
        recvdata = comm.bcast(recvdata,root=0) 
    #b=MPI.Wtime()
    #print(b-a)
    ...
    #zp=MPI.Wtime()
    if flagF1 == 0:
        cur = prefY11.dot(recvdata)
    if flagF1 == 1:
        cur = fY11.dot(recvdata)
    if flagF1 == 2:
        cur = posfY11.dot(recvdata)
    #w=MPI.Wtime()
    #print(w-zp)
    ...
    curdg = curd
    curqg = curq
    ed = edprime[:,S_Steps-1]+gen_dtr*curqg
    eq = eqprime[:,S_Steps-1]-gen_dtr*curdg
    ...
    dmac_spd[:,S_Steps-1] = (pmech[:,S_Steps-1]-pelect[:,S_Steps-1]-gen_do*(mac_spd[:,S_Steps-1]-1.0))/(2.0*gen_H)
    ...
    if S_Steps == 1:
        ...
        k2_mac_spd = h_sol1*(pmech[:,S_Steps-1]-pelect[:,S_Steps-1]-gen_do*(mac_spd[:,S_Steps-1]+k1_mac_spd-1.0))/(2.0*gen_H)
        ...
        mac_spd[:,S_Steps] = mac_spd[:,S_Steps-1]+h_sol1*(3*(pmech[:,S_Steps-1]-pelect[:,S_Steps-1]-gen_do*(mac_spd[:,S_Steps-1]-1.0))/(2.0*gen_H)-dmac_spd[:,S_Steps-2])/2.0 
    
    edprime[:,S_Steps] = edprime[:,S_Steps-1]
    eqprime[:,S_Steps] = eqprime[:,S_Steps-1]
