...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        REAL (KIND=DP)::k1_mac_ang,k1_mac_spd,k2_mac_ang,k2_mac_spd
...
!$OMP DO PRIVATE(k)
           DO k=1,ngen,1
              ...
              IF (S_Steps == 1) THEN
                 ! k1
                 k1_mac_ang=h_sol1*dmac_ang(k,S_Steps)
                 k1_mac_spd=h_sol1*dmac_spd(k,S_Steps)
                 ! k2
                 k2_mac_ang=h_sol1*(basrad*((mac_spd(k,S_Steps)+k1_mac_ang)-1.0))
                 k2_mac_spd=h_sol1*(pmech(k,S_Steps)-pelect(k,S_Steps)*mac_con(k,3)-mac_con(k,17)*(&
                            mac_spd(k,S_Steps)+k1_mac_spd-1.0))/(2.0*mac_con(k,16))
                 ! y1
                 mac_ang(k,S_Steps+1)=mac_ang(k,S_Steps)+h_sol1*(k1_mac_ang+k2_mac_ang)/2.0
                 mac_spd(k,S_Steps+1)=mac_spd(k,S_Steps)+h_sol1*(k1_mac_spd+k2_mac_spd)/2.0
              
              ELSE IF (S_Steps > 1) THEN
                 mac_ang(k,S_Steps+1)=mac_ang(k,S_Steps)+h_sol1*(3*basrad*(mac_spd(k,S_Steps)-1.0)-dmac_ang(k,S_Steps-1))/2.0
                 mac_spd(k,S_Steps+1)=mac_spd(k,S_Steps)+h_sol1*(3*(pmech(k,S_Steps)-pelect(k,S_Steps)*mac_con(k,3)-mac_con(k,17)*(&
                                      mac_spd(k,S_Steps)-1.0))/(2.0*mac_con(k,16))-dmac_spd(k,S_Steps-1))/2.0 
              ENDIF
              !IF (k==1) THEN
              !   PRINT *,I_Steps,S_Steps,&
